...
// QueryRoomstatusRoom chains the current query on the RoomstatusRoom edge.
func (rsq *RoomStatusQuery) QueryRoomstatusRoom() *RoomQuery {
...
//  WithRoomstatusRoom tells the query-builder to eager-loads the nodes that are connected to
// the "RoomstatusRoom" edge. The optional arguments used to configure the query builder of the edge.
func (rsq *RoomStatusQuery) WithRoomstatusRoom(opts ...func(*RoomQuery)) *RoomStatusQuery {
	query := &RoomQuery{config: rsq.config}
	for _, opt := range opts {
		opt(query)
	}
	rsq.withRoomstatusRoom = query
	return rsq
}
...
func (rsq *RoomStatusQuery) sqlAll(ctx context.Context) ([]*RoomStatus, error) {
	...
	if query := rsq.withRoomstatusRoom; query != nil {
		...
		query.Where(predicate.Room(func(s *sql.Selector) {
			s.Where(sql.InValues(roomstatus.RoomstatusRoomColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.room_status_roomstatus_room
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "room_status_roomstatus_room" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_status_roomstatus_room" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.RoomstatusRoom = append(node.Edges.RoomstatusRoom, n)
		}
	}
