...
// QueryUserRent chains the current query on the UserRent edge.
func (uq *UserQuery) QueryUserRent() *RentQuery {
	query := &RentQuery{config: uq.config}
...
// QueryUserUserstatus chains the current query on the UserUserstatus edge.
...
//  WithUserRent tells the query-builder to eager-loads the nodes that are connected to
// the "UserRent" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithUserRent(opts ...func(*RentQuery)) *UserQuery {
	query := &RentQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserRent = query
	return uq
}

//  WithUserUserstatus tells the query-builder to eager-loads the nodes that are connected to
// the "UserUserstatus" edge. The optional arguments used to configure the query builder of the edge.
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withUserRent; query != nil {
		...
		query.Where(predicate.Rent(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UserRentColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_user_rent
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_user_rent" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_user_rent" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.UserRent = append(node.Edges.UserRent, n)
		}
	}
