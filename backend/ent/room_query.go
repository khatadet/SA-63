...
type RoomQuery struct {
	...
	// eager-loading edges.
	withRoomRoomstatus *RoomStatusQuery
	withRoomRoomtype   *RoomTypeQuery
	withRoomRent       *RentQuery
	withFKs            bool
...
// QueryRoomRoomstatus chains the current query on the RoomRoomstatus edge.
func (rq *RoomQuery) QueryRoomRoomstatus() *RoomStatusQuery {
...
// QueryRoomRoomtype chains the current query on the RoomRoomtype edge.
...
// QueryRoomRent chains the current query on the RoomRent edge.
func (rq *RoomQuery) QueryRoomRent() *RentQuery {
...
//  WithRoomRoomstatus tells the query-builder to eager-loads the nodes that are connected to
// the "RoomRoomstatus" edge. The optional arguments used to configure the query builder of the edge.
func (rq *RoomQuery) WithRoomRoomstatus(opts ...func(*RoomStatusQuery)) *RoomQuery {
	query := &RoomStatusQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withRoomRoomstatus = query
	return rq
}

//  WithRoomRoomtype tells the query-builder to eager-loads the nodes that are connected to
// the "RoomRoomtype" edge. The optional arguments used to configure the query builder of the edge.
...
//  WithRoomRent tells the query-builder to eager-loads the nodes that are connected to
// the "RoomRent" edge. The optional arguments used to configure the query builder of the edge.
func (rq *RoomQuery) WithRoomRent(opts ...func(*RentQuery)) *RoomQuery {
	query := &RentQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withRoomRent = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context) ([]*Room, error) {
	var (
		...
		_spec       = rq.querySpec()
		loadedTypes = [3]bool{
			rq.withRoomRoomstatus != nil,
			rq.withRoomRoomtype != nil,
			rq.withRoomRent != nil,
		}
	)
	if rq.withRoomRoomstatus != nil || rq.withRoomRoomtype != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := rq.withRoomRoomstatus; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Room)
		for i := range nodes {
			if fk := nodes[i].room_status_roomstatus_room; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_status_roomstatus_room" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.RoomRoomstatus = n
			}
		}
	}
	...
	if query := rq.withRoomRent; query != nil {
		...
		query.Where(predicate.Rent(func(s *sql.Selector) {
			s.Where(sql.InValues(room.RoomRentColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.room_room_rent
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "room_room_rent" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_room_rent" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.RoomRent = append(node.Edges.RoomRent, n)
		}
	}
