...
type RentQuery struct {
	...
	// eager-loading edges.
	withRentRoom      *RoomQuery
	withRentUser      *UserQuery
	withRentInsurance *InsuranceQuery
	withRentRoomage   *RoomageQuery
	withFKs           bool
...
// QueryRentRoom chains the current query on the RentRoom edge.
func (rq *RentQuery) QueryRentRoom() *RoomQuery {
...
// QueryRentUser chains the current query on the RentUser edge.
...
// QueryRentInsurance chains the current query on the RentInsurance edge.
func (rq *RentQuery) QueryRentInsurance() *InsuranceQuery {
	query := &InsuranceQuery{config: rq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryRentRoomage chains the current query on the RentRoomage edge.
func (rq *RentQuery) QueryRentRoomage() *RoomageQuery {
	query := &RoomageQuery{config: rq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
//  WithRentRoom tells the query-builder to eager-loads the nodes that are connected to
// the "RentRoom" edge. The optional arguments used to configure the query builder of the edge.
func (rq *RentQuery) WithRentRoom(opts ...func(*RoomQuery)) *RentQuery {
	query := &RoomQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withRentRoom = query
	return rq
}

//  WithRentUser tells the query-builder to eager-loads the nodes that are connected to
// the "RentUser" edge. The optional arguments used to configure the query builder of the edge.
func (rq *RentQuery) WithRentUser(opts ...func(*UserQuery)) *RentQuery {
	query := &UserQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withRentUser = query
	return rq
}

//  WithRentInsurance tells the query-builder to eager-loads the nodes that are connected to
// the "RentInsurance" edge. The optional arguments used to configure the query builder of the edge.
func (rq *RentQuery) WithRentInsurance(opts ...func(*InsuranceQuery)) *RentQuery {
	query := &InsuranceQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withRentInsurance = query
	return rq
}

//  WithRentRoomage tells the query-builder to eager-loads the nodes that are connected to
// the "RentRoomage" edge. The optional arguments used to configure the query builder of the edge.
func (rq *RentQuery) WithRentRoomage(opts ...func(*RoomageQuery)) *RentQuery {
	query := &RoomageQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withRentRoomage = query
	return rq
}
...
func (rq *RentQuery) sqlAll(ctx context.Context) ([]*Rent, error) {
	var (
		...
		loadedTypes = [4]bool{
			rq.withRentRoom != nil,
			rq.withRentUser != nil,
			rq.withRentInsurance != nil,
			rq.withRentRoomage != nil,
		}
	)
	if rq.withRentRoom != nil || rq.withRentUser != nil || rq.withRentInsurance != nil || rq.withRentRoomage != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := rq.withRentRoom; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Rent)
		for i := range nodes {
			if fk := nodes[i].room_room_rent; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_room_rent" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.RentRoom = n
			}
		}
	}

	if query := rq.withRentUser; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Rent)
		for i := range nodes {
			if fk := nodes[i].user_user_rent; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_user_rent" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.RentUser = n
			}
		}
	}

	if query := rq.withRentInsurance; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Rent)
		for i := range nodes {
			if fk := nodes[i].insurance_insurance_rent; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "insurance_insurance_rent" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.RentInsurance = n
			}
		}
	}

	if query := rq.withRentRoomage; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Rent)
		for i := range nodes {
			if fk := nodes[i].roomage_roomage_rent; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(roomage.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "roomage_roomage_rent" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.RentRoomage = n
			}
		}
	}
