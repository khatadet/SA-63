...
// The RoomageQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type RoomageQueryRuleFunc func(context.Context, *ent.RoomageQuery) error

// EvalQuery return f(ctx, q).
func (f RoomageQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.RoomageQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.RoomageQuery", q)
}

// The RoomageMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type RoomageMutationRuleFunc func(context.Context, *ent.RoomageMutation) error

// EvalMutation calls f(ctx, m).
func (f RoomageMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.RoomageMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.RoomageMutation", m)
}
