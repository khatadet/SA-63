...
func (rsc *RoomStatusCreate) AddRoomstatusRoomIDs(ids ...int) *RoomStatusCreate {
	rsc.mutation.AddRoomstatusRoomIDs(ids...)
	return rsc
}

// AddRoomstatusRoom adds the RoomstatusRoom edges to Room.
func (rsc *RoomStatusCreate) AddRoomstatusRoom(r ...*Room) *RoomStatusCreate {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
	return rsc.AddRoomstatusRoomIDs(ids...)
}
...
func (rsc *RoomStatusCreate) createSpec() (*RoomStatus, *sqlgraph.CreateSpec) {
	...
	if nodes := rsc.mutation.RoomstatusRoomIDs(); len(nodes) > 0 {
